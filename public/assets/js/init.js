...
$(document).ready(function () {
  ...
  var $win = $(window);
  if ($win.scrollTop() == 0) jQuery(".navbar-fixed-top").addClass("wow");
  ...
  $("#search-wrapper, #search-wrapper button.close").on(
    "click keyup",
    function (event) {
      if (
        event.target == this ||
  ...
  if (masonry_portfolio_selectors != "undefined") {
    ...
    masonry_portfolio.imagesLoaded(function () {
      masonry_portfolio.isotope({
        ...
        masonry: {
          columnWidth:
            masonry_container.width() /
            masonry_container.attr("data-masonry-cols"),
        },
      });
    });
